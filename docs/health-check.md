...
The path and threshold are optional; defaults are `/` and `75`. A reading equal to the threshold is healthy. Exit codes: 0 healthy, 1 warning, 2 invalid input or disk inspection error. Unexpected failures of system utilities also terminate the script with a nonzero status.
